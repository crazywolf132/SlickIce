...
$introMsg = t('To install SlickIce, please fill out the form below.');
...
<form action="<?=$view->url('/install', 'configure')?>" method="post">
	...
	<div style="text-align: center">
	<button class="btn btn-lg btn-primary" type="submit"><?=t('Install SlickIce')?> <i class="fa fa-thumbs-up fa-white"></i></button>
	</div>
	<br/><br/>
...
<tr>
	...
	<td><i id="ccm-test-cookies-enabled-tooltip" class="fa fa-question-circle launch-tooltip" title="<?=t('Cookies must be enabled in your browser to install SlickIce.')?>"></i></td>
</tr>
<tr>
    <td><?php if ($i18nTest) {
    ?><i class="fa fa-check"></i><?php
...
<div class="col-sm-5">
    ...
    ?>
                            ...
                            Your memory limit is currently %sMB. You may experience problems uploading and resizing large images, and may have to install SlickIce without sample content.', round(Core::make('helper/number')->formatSize($memoryBytes, 'MB'), 2))?></span>
                        <?php
}
    ?>
								<?php if ($memoryTest === 1) {
